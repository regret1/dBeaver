...
--			실행 후에는 테이블의 ROW(행)가 증가 한다.

-- INSRT INTO 테이블명 [(컬럼명,컬럼명,...)] VALUES(값,값,...); 
...
--SQL Error [1] [23000]: ORA-00001: unique constraint (MULTI.PK_DEPT_CP2) violated
INSERT INTO DEPT_COPY2 VALUES('D9','샘플부서','L1','한국');

--SQL Error [12899] [72000]: ORA-12899: value too large for column "MULTI"."DEPT_COPY2"."DEPT_ID" (actual: 3, maximum: 2)
INSERT INTO DEPT_COPY2 VALUES('D10','샘플부서','L1','한국');

--컬럼 수정
ALTER TABLE DEPT_COPY2 MODIFY DEPT_ID CHAR(3);
ALTER TABLE DEPT_COPY2 MODIFY DEPT_TITLE VARCHAR2(30);
ALTER TABLE DEPT_COPY2 MODIFY LOCATION_ID VARCHAR2(2);
ALTER TABLE DEPT_COPY2 MODIFY LNAME CHAR(20);

-- DEPT_ID 컬럼의 크기를 3으로 변경 후에는  INSERT 잘 된다.
INSERT INTO DEPT_COPY2 VALUES('D10','샘플부서','L1','한국');

SELECT * FROM DEPT_COPY2;

--SQL Error [1441] [72000]: ORA-01441: cannot decrease column length because some value is too big
ALTER TABLE DEPT_COPY2
...
-- COMMIT : 트랜잭션이 종료될 때 정상적으로 종료되었다면 변경한 사항을 영구히 DB에 저장.
-- ROLLBACK : 트랜잭션 작업 중 오류가 발생할 경우 작업한 내역을 취소(복구).
--			  가장 최근에 COMMIT했던 시점으로 ROLLBACK;

-- DBEAVER는 자동으로 COMMIT해줌
